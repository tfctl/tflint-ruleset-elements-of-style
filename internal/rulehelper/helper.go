...
// normalizeBlock extracts the type, name, and synonym from a block.
...
// WalkBlocks iterates over blocks and locals using AST and applies the check function.
func WalkBlocks[T any](
	...
	for _, file := range files {
		...
		for _, block := range body.Blocks {
			...
			// Calculate a reasonable range for the definition.
			// Ideally we want the range of the type and labels.
			// hclsyntax.Block doesn't have a single DefRange, but we can construct one
			// or just use the block's range (which includes the body).
			// For style linting, usually the header is what we care about.
			// Let's try to construct it from TypeRange and LabelRanges.
